...
# repo init extra args (space-separated, forwarded as-is)
repo_init_extra_args=()
if [ -n "${REPO_INIT_ARGS-}" ]; then
  # shellcheck disable=SC2206
  repo_init_extra_args+=( $REPO_INIT_ARGS )
fi
...
if [ "$RESET_VENDOR_UNDO_PATCHES" = true ]; then
  ...
  for path in "vendor/cm" "vendor/lineage" "vendor/axion" "frameworks/base" "packages/apps/PermissionController" "packages/modules/Permission"; do
    if [ -d "$path" ]; then
      cd "$path"
      git reset -q --hard
      git clean -q -fd
...
for codename in ${devices//,/ }; do
  if [ -n "$codename" ]; then
    ...
    if [ "$CALL_REPO_INIT" = true ]; then
      ...
      if [ "$LOCAL_MIRROR" = true ]; then
        ( yes||: ) | repo init -u "$manifest_url" --reference "$MIRROR_DIR" -b "$branch" "${repo_init_extra_args[@]}" --git-lfs &>> "$repo_log"
      else
        ( yes||: ) | repo init -u "$manifest_url" -b "$branch" "${repo_init_extra_args[@]}" --git-lfs &>> "$repo_log"
      fi
    else
    ...
    # Determine vendor tree (Axion uses vendor/axion; some derivatives still use vendor/lineage)
    if [ ! -d "vendor/$vendor" ] && [ -d "vendor/lineage" ]; then
      echo ">> [$(date)] vendor/$vendor not found; falling back to vendor/lineage" | tee -a "$repo_log"
      vendor="lineage"
    fi
  ...
  # Setup our overlays
    ...
    overlay_dir="vendor/$vendor/overlay/custom"
    common_mk="vendor/$vendor/config/common.mk"
    mkdir -p "$overlay_dir"
    if [ -f "$common_mk" ]; then
      overlay_line="PRODUCT_PACKAGE_OVERLAYS := $overlay_dir"
      if ! grep -qF "$overlay_line" "$common_mk"; then
        sed -i "1s;^;$overlay_line\n;" "$common_mk"
      fi
    fi
    ...
    if [ -n "$OTA_URL" ]; then
      ...
      updater_url_overlay_dir="${overlay_dir}/${updater_values_dir}/"
    ...
    if [ -n "$CUSTOM_PACKAGES" ]; then
      ...
      if [ -f "$common_mk" ]; then
        ...
        echo ">> [$(date)] WARNING: $common_mk not found; cannot add CUSTOM_PACKAGES" | tee -a "$DEBUG_LOG"
      fi
    fi
    ...
    if [ "$SIGN_BUILDS" = true ]; then
      ...
      if [ "$android_version_major" -lt "10" ]; then
        if [ -f "$common_mk" ]; then
          ...
          echo ">> [$(date)] WARNING: $common_mk not found; cannot set signing keys in common.mk" | tee -a "$DEBUG_LOG"
        fi
      fi
    ...
    # Lunch / configure build target (Axion provides `axion`; fall back to Lineage `breakfast`)
    lunch_returncode=0
    if [ "$CALL_BREAKFAST" = true ]; then
      set +eu
      if declare -F axion >/dev/null 2>&1; then
        if [ -n "${AXION_VARIANT-}" ]; then
          # shellcheck disable=SC2086
          axion "$codename" $AXION_VARIANT &>> "$DEBUG_LOG"
        else
          if [ "$WITH_GMS" = true ]; then
            axion "$codename" gms "${AXION_GMS_VARIANT:-core}" &>> "$DEBUG_LOG"
          else
            axion "$codename" va &>> "$DEBUG_LOG"
          fi
        fi
      else
      ...
      echo ">> [$(date)] Lunch step disabled"
    fi

    if [ $lunch_returncode -ne 0 ]; then
        echo ">> [$(date)] lunch failed for $codename, $branch branch" | tee -a "$DEBUG_LOG"
        if [ -f /root/userscripts/post-build.sh ]; then
          echo ">> [$(date)] Running post-build.sh for $codename" >> "$DEBUG_LOG"
          /root/userscripts/post-build.sh "$codename" false "$branch" &>> "$DEBUG_LOG" || echo ">> [$(date)] Warning: post-build.sh failed!"
        fi
    ...
    if [ "$CALL_MKA" = true ]; then
      ...
      build_rc=0
      set +eu
      mka "${jobs_arg[@]}" target-files-package bacon &>> "$DEBUG_LOG"
      build_rc=$?
      set -eu
      ...
      if [ $build_rc -eq 0 ]; then
        ...
        for image in recovery boot vendor_boot dtbo super_empty vbmeta vendor_kernel_boot init_boot; do
          if [ -f "$image.img" ]; then
            rom_prefix=${ROM_NAME_PREFIX:-$vendor}
            recovery_name="${rom_prefix}-$los_ver-$builddate-$RELEASE_TYPE-$codename-$image.img"
